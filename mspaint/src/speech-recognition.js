...
if (speech_recognition_available) {
	...
	recognition.onnomatch = function () {
		...
		$status_text.text(localize("Speech not recognized."));
	};
	...
	recognition.onerror = function (event) {
		if (event.error.toString().match(/no-speech/)) {
			...
			$status_text.text(localize("Error occurred in speech recognition: ") + event.error);
	...
	trace_and_sketch = (subject_imagedata) => {
		...
		$status_text.text(localize(`To stop drawing, ${speech_recognition_active ? `say "stop drawing", or ` : ""}press Esc.`));
	...
	function find_clipart_and_sketch(subject_matter) {
		find_clipart(subject_matter).then((results) => {
			...
			img.onerror = () => {
				$status_text.text(localize("Failed to load clipart."));
			};
			img.onload = () => {
