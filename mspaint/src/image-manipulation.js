...
function stretch_and_skew(x_scale, y_scale, h_skew, v_skew) {
	apply_image_transformation({
		...
		// 添加尺寸限制，防止内存不足
		const max_dimension = 16384; // 浏览器Canvas最大尺寸限制
		if (w > max_dimension || h > max_dimension) {
			throw new Error("Image dimensions too large for operation");
		}
		...
		// 检查边界框是否有效
		if (!isFinite(bb_min_x) || !isFinite(bb_max_x) || !isFinite(bb_min_y) || !isFinite(bb_max_y)) {
		...
		// 检查最终尺寸是否合理
		if (bb_w <= 0 || bb_h <= 0 || bb_w > max_dimension || bb_h > max_dimension) {
