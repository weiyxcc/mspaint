...
function choose_color(initial_color, callback) {
	...
	$left.append(`<label for="basic-colors">${render_access_key("&" + localize("Basic colors") + ":")}</label>`);
	const $basic_colors_grid = make_color_grid(basic_colors, "basic-colors").appendTo($left);
	$left.append(`<label for="custom-colors">${render_access_key("&" + localize("Custom colors") + ":")}</label>`);
	...
	const $define_custom_colors_button = $(`<button class="define-custom-colors-button" type="button">`)
		.html(render_access_key("&" + localize("Define Custom Colors") + " >>"))
		.appendTo($left)
		.on("click", (e) => {
	...
	const $color_solid_label = $(`<label for="color-solid-canvas">${render_access_key(localize("Color") + " " + localize("Solid"))}</label>`);
	...
	const result_canvas = make_canvas(40, 30);
	...
	const $add_to_custom_colors_button = $(`<button class="add-to-custom-colors-button" type="button">`)
		.html(render_access_key("&" + localize("Add To Custom Colors")))
		.appendTo($right)
		.on("click", (event) => {
			// prevent the form from submitting
			// @TODO: instead, prevent the form's submit event in $Window.js in os-gui (or don't have a form? idk)
