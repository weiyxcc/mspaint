...
import { $G, E, TAU, debounce, from_canvas_coords, get_icon_for_tool, get_rgba_from_color, is_discord_embed, make_canvas, render_access_key, to_canvas_coords } from "./helpers.js";
 ...
 * @param {HTMLImageElement |HTMLCanvasElement | null=} options.icon - a visual representation of the operation type, shown in the history window, e.g. null
 * @returns {HistoryNode}
 */
function make_history_node({
	...
	icon = null, // an Image representation of the operation type, shown in the history window, e.g. null
}) {
	return {
		parent,
		futures,
...
async function load_image_from_uri(uri) {
	...
	for (let index_to_try = 0; index_to_try < uris_to_try.length; index_to_try += 1) {
		...
		try {
			if (is_download) {
				$status_text.text(localize("Downloading picture..."));
			}
			...
			if (is_download) {
				...
				$status_text.text(localize("Download complete."));
			}
	...
	if (is_download) {
		$status_text.text(localize("Failed to download picture."));
	}
...
function open_from_image_info(info, callback, canceled, into_existing_session, from_session_load) {
	are_you_sure(({ canvas_modified_while_loading } = {}) => {
		...
		current_history_node.image_data = main_ctx.getImageData(0, 0, main_canvas.width, main_canvas.height);
		current_history_node.icon = null;
...
const confirmed_overwrite_key = "mspaint confirmed overwrite capable";
try {
...
function show_about_paint() {
	...
	$("#mspaint-update-status-area").removeAttr("hidden");
...
function image_attributes() {
	...
	const table = {
		[localize("Resolution:")]: localize("72 x 72 dots per inch"), // if localizing this, remove "direction" setting below
	};
...
function image_stretch_and_skew() {
	...
	const $RowInput = ($table, img_src, label_with_hotkey, default_value, label_unit, min, max) => {
		...
		const access_key = AccessKeys.get(label_with_hotkey);
		const $input = $(E("input")).attr({
			...
			value: default_value,
			id: input_id,
			"aria-keyshortcuts": access_key ? `Alt+${access_key.toUpperCase()}` : "",
		}).css({
			width: "40px",
	...
	$w.$Button(localize("OK"), () => {
		...
		try {
			...
			if (exception.name === "NS_ERROR_FAILURE") {
				// or localize("There is not enough memory or resources to complete operation.")
				show_error_message(localize("Insufficient memory to perform operation."), exception);
			} else if (exception.message === "Image dimensions too large for operation") {
				show_error_message(localize("Image dimensions too large for operation."), exception);
			} else if (exception.message === "Invalid transformation parameters") {
				show_error_message(localize("Invalid transformation parameters."), exception);
			} else if (exception.message === "Invalid transformation result") {
				show_error_message(localize("Invalid transformation result."), exception);
			} else if (exception.message === "Invalid output dimensions") {
				show_error_message(localize("Invalid output dimensions."), exception);
			} else {
				show_error_message(localize("An unknown error has occurred."), exception);
			}
			// @TODO: undo and clean up undoable
